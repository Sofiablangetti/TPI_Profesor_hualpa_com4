...
Code → Download ZIP
Descomprimir la carpeta y abrirla en tu editor
...
Flujo de Inicio
El sistema verifica/crea src/base_de_datos/colegios.csv
...
║  3. ❌ Cancelar y salir                                  ║
╚══════════════════════════════════════════════════════════╝
Si eliges API, verifica /health con estado_servidor() en http://149.50.150.15:8020
Si el servidor no está disponible, vuelve automáticamente a modo local
📋 Menú Principal
╔══════════════════════════════════════════════════════════╗
...
🔄 Flujo de Datos
Modo Local
Lectura: utilidades.leer_csv() → Lee src/base_de_datos/colegios.csv y retorna lista de diccionarios con estructura:
...
CRUD en memoria: Funciones en carga_datos.py (agregar, editar, borrar)

Persistencia: utilidades.escribir_csv() → Guarda cambios en src/base_de_datos/colegios.csv

Modo API
Cliente HTTP: cliente_api.py realiza peticiones al servidor:
...
Lógica de API: modo_api.py contiene funciones que usan cliente_api y reutilizan funciones de vista.py, busqueda.py, estadisticas.py con los datos obtenidos de la API

Esquema directo: La API retorna y acepta datos con los campos reales del CSV (Provincia, Colegio, Cantidad de Estudiantes, Año de Creación) sin conversiones ni mapeos
...
Asegurarse de que src/funciones/__init__.py existe
Recargar la ventana del IDE: Ctrl+Shift+P → Developer: Reload Window
...
Verificar que el servidor esté corriendo: curl http://149.50.150.15:8020/health
El sistema vuelve automáticamente a modo local si el servidor no está disponible
Error: "No hay colegios en el archivo CSV local"
Causa: El archivo CSV está vacío o no existe.

Solución:

El sistema crea automáticamente el CSV si no existe
Agregar colegios usando la opción "Registrar nuevo colegio" del menú
El CSV no se guarda correctamente
Causa: Problemas de permisos o codificación.

Solución:

Verificar permisos de escritura en src/base_de_datos/
El CSV se guarda con codificación UTF-8 con BOM (compatible con Windows)
👥 Participación de Integrantes
...
🎥 Video Integrador
[Espacio reservado para el enlace al video integrador]
